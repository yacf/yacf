  ...
  <div style="float: right;">
    ...
    <b-modal v-bind:id="challenge.id" v-bind:title="title" ok-title="Confirm Remove" ok-variant="danger" @ok="handleOk">
      <p>
        ...
        <strong>delete {{challenge.name}}</strong>? This action cannot be undone. If any team has solved this challenge their solves will also be removed. Type in the challenge name below to confirm the deletion.
      </p>
  ...
  methods: {
    handleOk(evt) {
      ...
      if (this.confirm === this.challenge.name) {
        api(
          `mutation { removeChallenge(id:${this.challenge.id}){ code } }`
        ).then(data => {
        ...
        that.message = "Challenge was not deleted. Confirmation incorrect";
      }
    }
  }
};
