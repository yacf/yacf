            ...
            <tr v-for="user in users.edges" v-bind:key="user.id">
              ...
              <td>
                <div>
                  ...
                  <router-link tag="button" class="btn btn-secondary btn-sm" style="float: right" :to="{ name: 'AdminUserView', query: { id: user.node.id } }"><b-icon icon="arrow-up-right"></b-icon></router-link>
                </div>
              </td>
            </tr>
          </tbody>
...
export default {
  ...
  computed: {
    pageview: function () {
     ...
     if (upper > this.users.totalCount) upper = this.users.totalCount;
     return `${lower}-${upper} of ${this.users.totalCount}`
    }
  },
  methods: {
    emitevent(value) {
