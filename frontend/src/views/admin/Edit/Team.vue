<template>
  <div style="padding:20px">
    <div v-if="team">
      ...
      <input class="form-control" v-model="accesscode" />
      <p>{{message}}</p>
...
export default {
  ...
  methods: {
    update() {
      let that = this;
      api(
        `mutation { updateteam(id:${this.team.id}, name:"${this.team.name}", affiliation:"${this.team.affiliation}", email:"${this.team.email}", website:"${this.team.website}", accesscode:"${this.accesscode}"){ message } }`
      ).then(data => {
